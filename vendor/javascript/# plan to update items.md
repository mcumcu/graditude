# plan to update items

You can also specify images and create products directly on the Checkout Session by using the product_data parameter. This can be useful when product data is dynamic or one-off.
see `line_items.price_data.product_data.*`
...
Possible enum values:
      ...
      - `line_items.data.price.external_reference` (string, nullable)
...
for each line item in the cart, plan to forward the runtime the runtime image, title, and description to the checkout session, as this data is dynamic and item-specific